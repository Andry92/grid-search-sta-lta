...
def training_phase(config_file):
    # open log_file in write mode
    log_filename = "training_log_"+str(datetime.datetime.now()).replace(":", "")+".log"
    print(log_filename)
    ...
    # for a combination of sta - lta list
    import itertools
    sta_list = range(config_file['sta_min_value'], config_file['sta_max_value'], 2)
    lta_list = range(config_file['lta_min_value'], config_file['lta_max_value'], 20)

    c = list(itertools.product(sta_list, lta_list))

    print("STA and LTA tuples: ", c)
    write_to_file("STA and LTA tuples: "+ str(c))

    # iteration of the combinations for sta lta windows
    for comb in c:
        sta = comb[0]
        lta = comb[1]
        ...
        trig_on_list = range(config_file['trig_on_min_value'], config_file['trig_on_max_value'], 2)
        trig_off_list = range(config_file['trig_off_min_value'], config_file['trig_off_max_value'], 2)

        c_trig = list(itertools.product(trig_on_list, trig_off_list))

        print("TRIGGER_ON and TRIGGER_OFF tuples: ", c_trig)
        write_to_file("TRIGGER_ON and TRIGGER_OFF tuples: "+ str(c_trig))

        # iteration of the combinations for trig_on trig_off thresholds
        for comb_trig in c_trig:
            ...
            for file_raw in all_files_raw:
                ...
                print("\nRAW DATA:", num_file_raw, file_raw)
                write_to_file("\nRAW DATA:" + str(num_file_raw) +" "+ file_raw)
    ...
    with open("results/training_results_"+end_time_formatted+".txt", "w") as quint_list_file:
        quint_list_file.write(str(quintuple_list))

    print("\nTraining results saved inside 'results' folder, filename: training_results_"+end_time_formatted+".txt")
...
if __name__ == "__main__":
    ...
    answer = input("""Are you sure to use the values setted in config_file.json?
Press Enter if you want to use them, otherwise Ctrl-C to stop the script""")

    if answer != "":
        print("Just press Enter")
        sys.exit()
    elif(config_file['sta_min_value'] > config_file['sta_max_value']):
        print("\nERROR: STA min value greater than STA max value")
        sys.exit()
    elif(config_file['lta_min_value'] > config_file['lta_max_value']):
        print("\nERROR: LTA min value greater than LTA max value")
        sys.exit()
    elif(config_file['trig_on_min_value'] > config_file['trig_on_max_value']):
        print("\nERROR: TRIGGER_ON min value greater than TRIGGER_ON max value")
        sys.exit()
    elif(config_file['trig_off_min_value'] > config_file['trig_off_max_value']):
        print("\nERROR: TRIGGER_OFF min value greater than TRIGGER_OFF max value")
        sys.exit()

    training_phase(config_file)